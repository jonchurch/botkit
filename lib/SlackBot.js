...
function Slackbot(configuration) {
    ...
    slack_botkit.createWebhookEndpoints = function(webserver, authenticationTokens) {
        ...
        webserver.post('/slack/receive', function(req, res) {
            ...
            if (req.body.type === 'event_callback' && slack_botkit.config.eventsApi) handleEventsAPI(req, res);
        ...
        // Start ticking here to enable conversations, even if RTM is not being used
        // Otherwise, we start ticking when RTM is opened
        if (slack_botkit.config.eventsApi) {
        ...
        function handleEventsAPI(req, res) {
            ...
            slack_botkit.findTeamById(message.team, function(err, team) {
                console.log('==========TEAM:\n', team)
                if (err || !team) {
                    slack_botkit.log.error('Could not load team:', err);
                    return;
                }
                ...
                if (!team.bot.identity) {
                  // Find bot identity, and save it to team storage
                    slack_botkit.log.error('Received Events API message, but could not identify bot');

                } else {
                    var bot = slack_botkit.spawn(team);
                    // Identify the bot from either team storage or identifyBot()
                    bot.team_info = team;
                    bot.identity = team.bot.identity;
                            ...
                            slack_botkit.findTeamById(identity.team_id, function(err, team) {
                                ...
                                slack_botkit.saveTeam(team, function(err, id) {
                                    if (err) {
                                        ...
                                        if (!team.bot.identity || !team.bot.identity.id || !team.bot.identity.name) {
                                        //   saveBotIdentity(team);
