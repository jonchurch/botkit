...
function linebot(configuration) {
	...
	var group = {
		getMember: getGroupMember(opts, cb) => {
			const groupId = opts.groupId
			const userId = opts.userId
			if (userId && groupId) {

				api_request.(`https://api.line/v2/bot/group/${groupId}/member/${userId}`, (err, res, body) => {
					if (err) {
		...
		listMembers: listMembers(opts, cb) => {
