...
function linebot(configuration) {
    ...
    line_botkit.middleware.ingest.use(function(bot, message, res, next) {
        ...
        message = { raw_message: clone(payload) };
		next()
	});
    ...
    line_botkit.middleware.normalize.use(function handleMessage(bot, message, next) {
        ...
        if (message_event) {
			...
			message.location = message.type === 'location' ? { title: message_event.title, address: message_event.address, latitude: message_event.latitude, longitude: message_event.longitude } : null;
        }
    ...
    line_botkit.middleware.normalize.use(function handlePostback(bot, message, next) {
        ...
        if (message_event.type === 'postback') {

            message.text = message_event.postback.data;
            message.payload = message_event.postback.data;
