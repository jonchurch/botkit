...
function linebot(configuration) {
    ...
    line_botkit.middleware.format.use(function(bot, message, platform_message, next) {
		...
		if (message.sticker) {
			...
			_msg.stickerId = message.sticker.stickerIdA
			_msg.type = 'sticker'
		}
    ...
    line_botkit.middleware.normalize.use(function normalizeMessage(bot, message, next) {
		...
		message.replyToken = message.message.replyToken || null;
    ...
    line_botkit.middleware.normalize.use(function handlePostback(bot, message, next) {
		...
		const postback_event = message.raw_message.postback

        if (postback_event) {

            message.text = postback_event.postback.data;
            message.payload = postback_event.postback.data;
			message.params = postback_event.postback.params
