...
function Telegrambot(configuration) {
            ...
            if (obj.entry) {
                for (var e = 0; e < obj.entry.length; e++) {
                    for (var m = 0; m < obj.entry[e].messaging.length; m++) {
                        var telegram_message = obj.entry[e].messaging[m];
                        if (telegram_message.message) {

                            var message = {
                                text: telegram_message.message.text,
                                user: telegram_message.sender.id,
                                channel: telegram_message.sender.id,
    ...
    telegram_botkit.setupWebserver = function(port, cb) {
        ...
        request.post('https://api.telegram.org/bot' + configuration.access_token + '/' + 'setWebhook', {url: configuration.webhook_url },
            function(err, res, body) {
                if (err) {
