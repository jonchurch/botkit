...
function Telegrambot(configuration) {
	...
	telegram_botkit.middleware.normalize.use(function handleMessage(bot, message, next) {
		...
		if (message.message) {
			...
			message.user = message.message.from.id
			message.channel = message.message.chat.id
    ...
    telegram_botkit.middleware.categorize.use(function categorize(bot, message, next) {
        ...
        if (message.message) {
			...
			} else if (message.message.type === 'group') {
				message.type = 'group_message'
			} else if (message.message.type === 'channel') {
				message.type = 'channel_message'
			} else if (message.message.type === 'supergroup') {
				message.type = 'supergroup_message'
			}
            console.log('Wooh! message.message is truthy:\n', message.message)
        }
		next()
    })
