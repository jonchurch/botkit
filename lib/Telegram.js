...
function Telegrambot(configuration) {
	...
	if (message.inline_query) {
		...
		if (message.chosen_inline_result) {
			...
			// inline query initially sent to bot
		...
		// @TODO need to answer callback queries with an API call
    ...
    telegram_botkit.middleware.categorize.use(function categorize(bot, message, next) {
        ...
        if (message.message && message.message.text) {
			...
			if (message.new_chat_members) {
				// @TODO check bot join and new member join events
				// We get sent an array of users, not when or how many at a time. Individual events would be useful here, but might not be an option. Find out! 
				...
				// not sure if we get an event when bot is kicked
