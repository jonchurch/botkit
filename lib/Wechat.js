...
function Wechatbot(configuration) {
    ...
    wechat_botkit.defineBot(function(botkit, config) {
        ...
        bot.send = function(message, cb) {
            ...
            request({
                ...
                body: wechat_message,
                uri: 'https://api.wechat.com/cgi-bin/message/custom/send?access_token=' + bot.config.access_token || configuration.access_token
            },
    ...
    wechat_botkit.createWebhookEndpoints = function(webserver, bot, cb) {
        ...
        wechat_botkit.log(
            ...
            'http://' + wechat_botkit.config.hostname + ':' + facebook_botkit.config.port + '/facebook/receive');
    ...
    wechat_botkit.setupWebserver = function(port, cb) {
        ...
        if (wechat_botkit.config && facebook_botkit.config.webserver && facebook_botkit.config.webserver.static_dir)
        ...
        request.post('https://' + api_host + '/me/subscribed_apps?access_token=' + configuration.access_token,
            function(err, res, body) {
                if (err) {
                    wechat_botkit.log('Could not subscribe to page messages');
                } else {
                    wechat_botkit.debug('Successfully subscribed to Facebook events:', body);
                    wechat_botkit.startTicking();
                }
            });
