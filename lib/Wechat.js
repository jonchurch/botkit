...
function Wechatbot(configuration) {
    ...
    wechat_botkit.defineBot(function(botkit, config) {
        ...
        bot.startConversation = function(message, cb) {
            wechat_botkit.startConversation(this, message, cb);
        };
        ...
        bot.send = function(message, cb) {
			...
			botkit.api.getAccessToken(function(err, accessToken){
				if (err) {
					botkit.log('Error getting access token:', err)
				} else {
					var wechat_message = {
					...
					request({
						...
						body: wechat_message,
						uri: 'https://api.wechat.com/cgi-bin/message/custom/send?access_token=' + accessToken
					},
    ...
    wechat_botkit.api = {
		getAccessToken: function(cb) {
			console.log(wechat_botkit.config)
			request({
                method: 'GET',
                json: true,
				uri: `https://api.wechat.com/cgi-bin/token?grant_type=client_credential&appid=${wechat_botkit.config.app_id}&secret=${wechat_botkit.config.app_secret}`,
			}, function(err, res, body){

				if (err) {
					wechat_botkit.debug('WEBHOOK ERROR', err);
					return cb && cb(err);
				}

				if (body.error) {
					wechat_botkit.debug('API ERROR', body.error);
					return cb && cb(body.error.message);
				}

				wechat_botkit.debug('API SUCCESS:', body);
				cb && cb(null, body.access_token);
			})
		}
    };
