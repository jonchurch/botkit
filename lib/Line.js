...
function linebot(configuration) {
    ...
    var api_host = configuration.api_host || 'graph.line.com';
        ...
        if (payload.entry) {
            for (var e = 0; e < payload.entry.length; e++) {
                for (var m = 0; m < payload.entry[e].messaging.length; m++) {
                    line_botkit.ingest(bot, payload.entry[e].messaging[m], res);
                }
            }
        }
    ...
    line_botkit.middleware.normalize.use(function normalizeMessage(bot, message, next) {
        ...
        // since there are only 1:1 channels on line, the channel id is set to the user id
    ...
    line_botkit.middleware.categorize.use(function handleOptIn(bot, message, next) {
        ...
        if (message.referral) {
            message.type = 'line_referral';
        }
        if (message.account_linking) {
            message.type = 'line_account_linking';
        }
        if (message.is_echo) {
