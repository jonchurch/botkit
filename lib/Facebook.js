...
function Facebookbot(configuration) {
    ...
    facebook_botkit.defineBot(function(botkit, config) {
        ...
        bot.send = function(message, cb) {
            ...
            facebook_message.access_token = this.config.access_token;
    ...
    facebook_botkit.handleWebhookPayload = function(req, res, botList) {

        var obj = req.body;
        if (obj.entry) {
            for (var e = 0; e < obj.entry.length; e++) {

                var pageId = obj.entry[e].id
                
                var botConfig = botList.find(function(config){
                    return config.id === pageId
                })

                var bot = spawn(botConfig)
