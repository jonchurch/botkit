...
function Facebookbot(configuration) {
    ...
    facebook_botkit.defineBot(function(botkit, config) {
        ...
        bot.send = function(message, cb) {
            ...
            message.access_token = configuration.access_token || bot.config.access_token
    ...
    facebook_botkit.handleWebhookPayload = function(req, res, bot) {
        ...
        // now I need to spawn a configured bot for each page
        // go check storage for page info

        // facebook may send more than 1 message payload at a time
        // we split these up into multiple message objects for ingestion
        if (payload.entry) {
            facebook_botkit.getBotConfig(payload).then(config => {
				console.log('Outside getBotConfig, config:',config)
				for (var e = 0; e < payload.entry.length; e++) {
					var page = payload.entry[e]
							var bot = facebook_botkit.spawn(config[page.id])
					console.log('Assoc lookup:',config[page.id])
							if (page.messaging) {
								for (var m = 0; m < page.messaging.length; m++) {
									facebook_botkit.ingest(bot, payload.entry[e].messaging[m], res);
								}

							}
						}
			}).catch(err => facebook_botkit.debug(err))
        }

    };

    // Configure bot based on the page message is from
    facebook_botkit.getBotConfig = function(payload) {
        const p = Promise.all(payload.entry.map(el => new Promise((resolve, reject) => {
            // Backwards compatible with single token bots
            // if (facebook_botkit.config.access_token) {
            //     resolve(facebook_botkit.config)
            // } else {
                // Go looking in storage for page config
                facebook_botkit.storage.teams.get(el.id, (err, config) => {
                    if (err) {
                        reject(err)
                    } else {
                        if (config.access_token) {
                            ...
                            facebook_botkit.log(`No access_token found for page record ${el.id}`)
                        }
                    }
                })
            // }
		...
		return new Promise((resolve, reject) => {
			p.then(pages => {
				console.log('PROMISE ALL RESOLVING', pages)
				const pageConfig = pages.reduce((a, el) => { 
				a[`${el.id}`] = el
				return a
			}, {})
    ...
    facebook_botkit.on('webserver_up', function(webserver) {
        ...
        if (facebook_botkit.config.validate_requests === true) {
            ...
            webserver.post('/facebook/receive', bodyParser.json({
