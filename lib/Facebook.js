...
function Facebookbot(configuration) {
    ...
    facebook_botkit.middleware.normalize.use(function normalizeMessage(bot, message, next) {
		...
		if (message.sender) {
			...
			console.log('=====Message with no sender!', {message})
			next()
		}
	...
	facebook_botkit.middleware.categorize.use(function handoverEvents(bot, message, next) {

		if (message.app_roles) {
			message.type = 'facebook_app_roles';
		} 
		if (message.standby) {
			message.type = 'facebook_standby'
		}
		if (message.pass_thread_control) {
			message.type = 'facebook_pass_thread_control'
		}
		if (message.take_thread_control) {
			message.type = 'facebook_take_thread_control'
		}
    ...
    var handover = {
        ...
        take_thread_control: function(user, metadata, cb) {
            var request_body = {
                recipient: {
                    id: user
                },
        ...
        pass_thread_control: function(user, target, metadata, cb) {
            var request_body = {
                recipient: {
                    id: user
                },
