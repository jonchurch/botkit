...
function Facebookbot(configuration) {
        ...
        if (payload.entry) {
            for (var e = 0; e < payload.entry.length; e++) {
				var page = payload.entry[e]
					facebook_botkit.getBotConfig(page.id)
					.then(config => {
						var bot = facebook_botkit.spawn(config)
						console.log('PAYLOAD MESSAGING:', payload.entry)
						if (page.messaging) {
	...
	facebook_botkit.getBotConfig = function(pageId) {
		return new Promise(function(resolve, reject) {
