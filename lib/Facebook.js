...
function Facebookbot(configuration) {
    ...
    var handover = {
        get_secondary_receivers_list: function(fields, cb) {
            var uri = 'https://' + api_host + '/v2.6/me/secondary_receivers?' + (fields ? 'fields=' + fields + '&' : '') + 'access_token=' + configuration.access_token;
            request.get(uri,
                function(err, res, body) {
                    if (err) {
                        ...
                        try {
                            ...
                            facebook_botkit.log('ERROR in secondary receivers list call: Could not parse JSON', err, body);
                        }
        ...
        take_thread_control: function(recipient, metadata, cb) {
            ...
            request.post('https://' + api_host + '/v2.6/me/take_thread_control?access_token=' + configuration.access_token,
                request_body,
                function(err, res, body) {
                    if (err) {
                        ...
                        try {
                            ...
                            facebook_botkit.log('ERROR in take thread control API call: Could not parse JSON', err, body);
                        }

                        if (results) {
                            if (results.error) {
                                facebook_botkit.log('ERROR in take thread control API call: ', results.error.message);
                            } else {
        ...
        pass_thread_control: function(recipient, target, metadata, cb) {
            ...
            request.post('https://' + api_host + '/v2.6/me/pass_thread_control?access_token=' + configuration.access_token,
                request_body,
                function(err, res, body) {
                    if (err) {
                        ...
                        try {
                            ...
                            facebook_botkit.log('ERROR in pass thread control API call: Could not parse JSON', err, body);
                        }

                        if (results) {
                            if (results.error) {
                                facebook_botkit.log('ERROR in pass thread control API call: ', results.error.message);
                            } else {
